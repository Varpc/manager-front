...
export default class AdminConfig extends React.Component {
  ...
  handleSubmit = () => {
    ...
    const data = {
      jisuanke_update_interval: Math.ceil(this.state.jisuanke_update_interval) * 3600,
      codeforces_update_interval: Math.ceil(this.state.codeforces_update_interval) * 3600,
      wait_time: Math.ceil(this.state.wait_time),
      try_time: Math.ceil(this.state.try_time),
    }
    axios.put('/api/admin/config', data).then((r) => {
